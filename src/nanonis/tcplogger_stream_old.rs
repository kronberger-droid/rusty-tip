...
impl TCPLoggerStream {
    ...
    /// Connect only to TCP data stream without control connection.
    /// 
    /// This method creates a TCPLoggerStream that only connects to the data stream port,
    /// without creating its own control connection. Control operations should be handled
    /// externally (e.g., via ActionDriver's NanonisClient).
    ///
    /// # Arguments
    /// * `addr` - Server address (e.g., "127.0.0.1")
    /// * `stream_port` - TCP Logger data stream port (typically 6590)
    ///
    /// # Returns
    /// Connected `TCPLoggerStream` ready to read data frames (no control).
    ///
    /// # Examples
    /// ```no_run
    /// use rusty_tip::TCPLoggerStream;
    /// 
    /// // Connect only to data stream - control handled elsewhere
    /// let stream = TCPLoggerStream::connect_stream_only("127.0.0.1", 6590)?;
    /// let receiver = stream.spawn_background_reader();
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn connect_stream_only(addr: &str, stream_port: u16) -> Result<Self, NanonisError> {
        // create the socket address
        let socket_addr: SocketAddr = format!("{addr}:{stream_port}")
            .parse()
            .map_err(|_| NanonisError::InvalidAddress(addr.to_string()))?;
        ...
        // Create a dummy client to satisfy the struct - this won't be used
        // TODO: Refactor struct to make control optional
        let dummy_control = NanonisClient::new(addr, 0)?; // Will fail but that's OK for now
