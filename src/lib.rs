...
impl NanonisValue {
    ...
    pub fn as_string_array(&self) -> Result<&[String], NanonisError> {
        match self {
            ...
            _ => Err(NanonisError::Type(format!("Expected string array, got {:?}", self))),
        }
    }
    ...
    pub fn as_f32_array(&self) -> Result<&[f32], NanonisError> {
        match self {
            ...
            _ => Err(NanonisError::Type(format!("Expected f32 array, got {:?}", self))),
        }
    }
}
...
impl NanonisClient {
    ...
    pub fn quick_send(
        ...
        if body.len() != body_types.len() {
            return Err(NanonisError::InvalidCommand(
                format!("Body length ({}) doesn't match body types length ({})", 
                       body.len(), body_types.len())
            ));
        }
    ...
    fn send(
        ...
        let response_body_size = u32::from_be_bytes([header[32], header[33], header[34], header[35]]) as usize;
        ...
        if bytes_read < response_body_size {
            warn!("Incomplete response: got {} bytes, expected {}", bytes_read, response_body_size);
        }

        response_body.truncate(bytes_read);

        if self.debug {
            debug!("Body size: {}, received: {}", response_body_size, bytes_read);
            trace!("Header: {:?}", header);
        ...
        match (value, body_type) {
            ...
            _ => {
                return Err(NanonisError::Type(format!(
                    "Unsupported type combination: {:?} with {}",
                    value, body_type
                )))
            }
        }
    ...
    pub fn set_xy_position(&mut self, position: Position, wait_end: bool) -> Result<(), NanonisError> {
        self.quick_send(
            "FolMe.XYPosSet",
            &[
    ...
    pub fn get_signal_names(&mut self) -> Result<Vec<String>, NanonisError> {
        ...
        match result.first() {
            ...
            None => Err(NanonisError::Protocol("No signal names returned".to_string())),
        }
    }
    ...
    pub fn signals_calibr_get(&mut self, signal_index: i32) -> Result<(f32, f32), NanonisError> {
        let result = self.quick_send("Signals.CalibrGet", &[NanonisValue::I32(signal_index)], &["i"], &["f", "f"])?;
        if result.len() >= 2 {
            Ok((result[0].as_f32()?, result[1].as_f32()?))
        } else {
            Err(NanonisError::Protocol("Invalid calibration response".to_string()))
        }
    }

    /// Get range limits of a signal by index
    pub fn signals_range_get(&mut self, signal_index: i32) -> Result<(f32, f32), NanonisError> {
        let result = self.quick_send("Signals.RangeGet", &[NanonisValue::I32(signal_index)], &["i"], &["f", "f"])?;
        if result.len() >= 2 {
    ...
    pub fn signals_val_get(&mut self, signal_index: i32, wait_for_newest_data: bool) -> Result<f32, NanonisError> {
        let wait_flag = if wait_for_newest_data { 1u32 } else { 0u32 };
        let result = self.quick_send(
            "Signals.ValGet", 
            &[NanonisValue::I32(signal_index), NanonisValue::U32(wait_flag)], 
            &["i", "I"], 
            &["f"]
        )?;
        match result.first() {
            Some(value) => Ok(value.as_f32()?),
            None => Err(NanonisError::Protocol("No signal value returned".to_string())),
        }
    }

    /// Get current values of multiple signals by indexes
    pub fn signals_vals_get(&mut self, signal_indexes: &[i32], wait_for_newest_data: bool) -> Result<Vec<f32>, NanonisError> {
        let wait_flag = if wait_for_newest_data { 1u32 } else { 0u32 };
        
        let result = self.quick_send(
            "Signals.ValsGet", 
            &[NanonisValue::ArrayI32(signal_indexes.to_vec()), NanonisValue::U32(wait_flag)], 
            &["+*i", "I"], 
            &["i", "*f"]
        )?;
        ...
        if result.len() >= 2 {
            match &result[1] {
                ...
                _ => Err(NanonisError::Protocol("Invalid signal values response".to_string())),
            }
        } else {
            Err(NanonisError::Protocol("Incomplete signal values response".to_string()))
        }
    }
    ...
    pub fn read_signal_by_name(&mut self, signal_name: &str, wait_for_newest: bool) -> Result<f32, NanonisError> {
        match self.find_signal_index(signal_name)? {
            ...
            None => Err(NanonisError::InvalidCommand(format!("Signal '{}' not found", signal_name))),
        }
    }
...
mod tests {
    ...
    fn test_value_type_safety() {
        let value = NanonisValue::F32(3.14);
