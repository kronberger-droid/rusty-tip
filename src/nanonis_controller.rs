    ...
    fn collect_tcp_samples(
        ...
        while collected.len() < num_samples && start.elapsed() < timeout {
            ...
            if !reader.is_buffering()
                && let Some(err_msg) = reader.stream_error()
            {
                return Err(SpmError::Io {
                    ...
                    context: format!("TCP data stream died during sample collection: {err_msg}"),
                });
            }
...
impl SpmController for NanonisController {
    ...
    fn auto_approach(&mut self, wait: bool, timeout: Duration) -> Result<()> {
        ...
        match self.client.auto_approach_on_off_get() {
            ...
            Err(e) => {
                log::warn!(
                    "Could not check auto-approach status: {}, proceeding anyway",
                    e
                );
            }
        }

        // Protocol errors here usually mean "already open" — harmless.
        // Connection errors mean the link is down — propagate immediately
        // so the caller doesn't chase a cascading failure downstream.
        match self.client.auto_approach_open() {
            Ok(_) => log::debug!("Opened auto-approach module"),
            Err(e) => {
                let spm: SpmError = e.into();
