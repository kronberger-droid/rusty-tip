...
use rusty_tip::{
    Amplitude, Frequency, MotorAxis, MotorDirection, MotorGroup, NanonisClient, Position,
    ScanFrame, SignalIndex, StepCount,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "Motor X-axis - Frequency: {:.2} Hz, Amplitude: {:.2} V",
        freq.0, amp.0
    );

    // Set new frequency and amplitude using type-safe wrappers
    client.motor_freq_amp_set(Frequency::hz(1000.0), Amplitude::volts(5.0), MotorAxis::X)?;
    ...
    println!(
        "Scan frame center: ({:.6}, {:.6}) m",
        scan_frame.center.x, scan_frame.center.y
    );
    println!(
        "Scan frame size: {:.6} x {:.6} m, angle: {:.1}°",
