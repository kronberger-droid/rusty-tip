...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let active_indices = [0, 24, 30]; // Current, Bias, Z
    let signal_values = [1.23e-12, 1.5, -1.07e-9];
    ...
    println!(
        "Metadata size: {} bytes",
        serde_json::to_string(&metadata)?.len()
    );
    ...
    println!(
        "For 10,000 samples: {:.1} KB vs {:.1} KB",
